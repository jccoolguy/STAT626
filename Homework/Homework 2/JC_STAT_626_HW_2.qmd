...
$$
E[w_tw_{t-1}]=E[w_t]E[w_{t-1}]=0
$$

Starting with $\gamma(t,t)$:

$$
\gamma(t,t)=Var(x_t)=Var(w_tw_{t-1})=E[(w_tw_{t-1}-\mu_{x_t})^2]=E[(w_tw_{t-1})^2]=E[w_t^2]E[w_{t-1}^2]=\sigma_w^4
$$

Then $\gamma(t+1,t)$:

$$
\gamma(t+1,t)=Cov(x_{t+1},x_t)=Cov(w_{t+1}w_t,w_tw_{t-1})
$$

$$
=E[(w_{t+1}w_t)(w_t w_{t-1})]=E[w_{t+1}w_t^2w_{t-1}]=E[w_{t+1}]E[w_t^2]E[w_{t-1}]=0
$$

And it will continue being 0 as lag increases so:

$$
\gamma(h)=\begin{cases} \sigma_w^4 & h =0 \\ 0 & h \geq1\end{cases}
$$
...
$$
x_t=.6x_{t-1}-.9 x_{t-2}+w_t
$$

```{r, echo=FALSE}
set.seed(121)
w = rnorm(250 + 50)
x = filter(w, filter = c(.6,-.9), method = "recursive")[-(1:50)]
tsplot(x, main="autoregression", col = 4)
y = filter(w, filter = c(1.5,-.75), method = "recursive")[-(1:50)]
lines(y)
```
...
These two time series have similar [magnitude]{.underline} and do not seem to have [drift]{.underline} as both vary around 0.

However they differ, the time series with my UIN digits as coefficients has faster [oscillations]{.underline} while the other time series is [smoother]{.underline} and looks like it has more defined [cycles.]{.underline}
...
$$
\gamma(t+1,t)=Cov(w_tw_{t-1}(w_{t+1}(t+1)),w_{t-1}w_{t-2}(w_t+t))
$$

$$
=Cov(w_tw_{t-1}w_{t+1}+w_tw_{t-1}(t+1),w_{t-1}w_{t-2}w_t+w_{t-1}w_{t-2}t)
$$

$$
=Cov(w_tw_{t-1}w_{t+1},w_{t-1}w_{t-2}w_t)+Cov(w_tw_{t-1}w_{t+1},w_{t-1}w_{t-2}t)
$$

$$
+Cov(w_tw_{t-1}(t+1),w_{t-1}w_{t-2}w_t)+Cov(w_tw_{t-1}(t+1),w_{t-1}w_{t-2}t)
$$

$$
=E[w_t^2w_{t-1}^2w_{t+1}w_{t-2}]+tE[w_tw_{t-1}^2w_{t+1}w_{t-2}]+(t+1)E[w_t^2w_{t-1}^2w_{t-2}]+t(t+1)E[w_tw_{t-1}^2w_{t-2}]
$$

Each of these terms have at least one $E[w_i]$, since $E[w_i]$ we end up with zero. So the autocovariance function is:

$$
\gamma(t,s)= \begin{cases} \sigma_w^6+t^2 \sigma_w^4 & s = t \\
0 & s\neq t\end{cases}
